...
mixin ApiUser {
  ...
  /// !需要登录
  /// [lastTime] 传入上一次返回结果的 lasttime,将会返回下一页的数据,默认-1
  Future<UserEventListWrap> userEventList(String userId,
      {int limit = 30, int lastTime = -1}) {
    return Https.dio
        .postUri(joinUri('/weapi/event/get/$userId'),
            ...
            options: joinOptions())
        .then((Response value) {
      return UserEventListWrap.fromJson(value.data);
    });
  }
